...
async function handleDoneMessage(data, context) {
  ...
  if (data.result && data.result.analysis_image_url) {
    ...
    // console.log('🖼️ DONE에서 분석 이미지 URL 설정:', context.langgraph.analysisImageUrl.value);
  }
  ...
  if (answerToAdd && context.$store.state.currentConversation) {
    ...
    try {
      ...
      console.log('✅ [DONE] LangGraph 메시지 저장 완료');
      
      // 대화 목록 새로고침 - 최신 메시지 반영
  ...
  // 랭그래프 완료 플래그 설정 (복원 방지) - 대화 목록 새로고침 전에 설정
